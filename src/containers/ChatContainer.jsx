...
function ChatContainer({
  ...
  function handleUpdateRoom(e) {
    ...
    updateRoom(data).then((response) => {
      ...
      const roomUpdated = rooms.filter((room) =>
        room._id === currentRoom._id ? room : null
      )[0];
  ...
  function hiddenDrop() {
    if (
      showDrop === "" ||
      showDrop === "profile-container" ||
      showDrop === "newchat-container"
    )
      return;
    setShowDrop("");
  }
          ...
          <Chat>
            ...
            {messages.map((message) => {
              return message.username === user.username ? (
                <Chat.MessageSender key={`${message.username}-${message._id}`}>
                  {message.message}
                  <Chat.TimeStamp>
                    {message.timestamp}
                    <IconButton
                ...
                <Chat.Message key={`${message.username}-${message._id}`}>
